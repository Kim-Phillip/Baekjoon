...
# import Foundation을 첫번째 방법 (16ms)
...
if let input = input {
    ...
    let addtion = Int(arrayInput[0])! + Int(arrayInput[1])!    //String?에서 String으로 강제 언랩핑 및 정수형으로 타입캐스팅
    print(addtion)
}
...
let arrayInput = input.components(separatedBy: " ")    //separator로 받은 string으로 나눠준 배열 리턴(components)
let addtion = Int(arrayInput[0])! + Int(arrayInput[1])!    //정수형으로 타입캐스팅 후 int?에서 int으로 강제 언랩핑
print(addtion)


# import Foundation을 안쓰는 방법 (8ms)

let input = readLine()!	//String?에서 String으로 강제 언랩핑
let arrayInput = input.split(separator: " ")	// separator로 받은 string값 공백 단위로 문자열 쪼개기(Split)
let firstInput = Int(arrayInput[0])!	//String 값 정수형으로 타입캐스팅
let secondInput = Int(arrayInput[1])!	//String 값 정수형으로 타입캐스팅
